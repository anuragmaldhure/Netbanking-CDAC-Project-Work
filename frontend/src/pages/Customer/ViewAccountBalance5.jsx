...
const ViewAccountBalance5 = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/Customer/Account/paginate/${3}?pageNumber=${pageNumber}&pageSize=${pageSize}`);
            ...
            <div style={{ "display": 'flex' }}>
                ...
                <div className='container' style={{ display: 'block', backgroundColor: 'lightcyan', ...backgroundStyle }}>
                            ...
                            <tbody>
                              {Array.isArray(transactions) && transactions.map((transaction, index) => (
                                <tr key={index}>
                                  ...
                                  <td>{transaction['transactionType'] === '+' ? transaction['transactionAmount'] : null}</td>
                                  <td>{transaction['transactionType'] === '-' ? transaction['transactionAmount'] : null}</td>
                                  <td>{/* Add the appropriate logic for this cell */}</td>
                    ...
                    <div style={{ textAlign: 'center', display: 'flex', gap: '20px', justifyContent: 'center' }}>
                        {/* Pagination buttons */}
                        <div>
                            <button onClick={loadPreviousTransactions} className='btn btn-primary' disabled={disablePreviousButton}>
                                Previous 7 transactions
                            </button>
                        </div>
                        <div>
                            <button onClick={loadNextTransactions} className='btn btn-info' disabled={disableNextButton}>
                                Next 7 transactions
                            </button>
                        </div>
                    </div>
